...
def f1_score(preds, refs):
    """
    code 는 맞게 구현됨 (f1 스코어를 nlp쪽에서 쓸 때 토큰단위로 함)
    Macro F1-Score
    """
    f1s = []
...
def calculate(raw_pred, raw_ref, data_name, task, logging, args):
    ...
    if task in ['resp', 'direct']:
        ...
        logging.info(f'Running BLEU for {data_name}')
        logging.info('BLEU: {}'.format(', '.join(map(str, bleu_scores))))

        dist_scores = list(distinct(preds))
        print('Running Dist for ' + ' ' + data_name + '-----------------------------')
        print('Dist: ', dist_scores)
        logging.info(f'Running Dist for {data_name}')
        logging.info('Dist: {}'.format(', '.join(map(str, dist_scores))))

        f1_scores = [f1_score(preds, refs)]
        print('Running F1 for ' + ' ' + data_name + '-----------------------------')
        print('F1: ', f1_scores)
        logging.info(f'Running F1 for {data_name}')
        logging.info('F1: {}'.format(', '.join(map(str, f1_scores))))
        ...
        logging.info('P/R/F1: {}'.format(', '.join(map(str, f1_scores))))
        logging.info('hits: {}'.format(', '.join(map(str, hit_scores))))

        auto_scores = f1_scores + hit_scores  # HJ hit_scores :: return [hits1, hits3, hits5]
        ...
        logging.info('Running NDCG and MRR for ' + ' ' + data_name + '-----------------------------')
        logging.info('NDCG@10/NDCG@50: {}'.format(', '.join(map(str, ndcg_scores))))
        logging.info('MRR@10/MRR@50: {}'.format(', '.join(map(str, mrr_scores))))
